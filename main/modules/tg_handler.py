...
async def start_uploading(data, source, header):
    ...
    title = data["title"] + f" [{source['quality']}p] [Sub] [ON]"
    link = source['url']
    ep_id = data["ep_id"]
    ...
    msg = await app.send_message(UPLOADS_ID,f"Gonna Download **{name}**")
    ...
    try:
        ...
        [
            InlineKeyboardButton(
                text="Otaku_Network", url=f"https://t.me/Otaku_Network"),
        ]
    ])
